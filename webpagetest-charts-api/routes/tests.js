...
/**
 * Auther: John Handley
 * Purpose: This gives a reference for aquiring teh agrigate info
 */
router.get('/:suiteId/stats/', function(req, res) {
  var data = getCache(req);
  ...
  if (!data) {
    ...
    data.tests.forEach(function(testName){
      testData = dataStore.getSuiteTest(data.suite, testName);
      data.charts.push(aggregateDataFromTests(
        data.suite,
        _.find(suiteConfig.testPages, {testId: testName}),
        testData.datapoints,
        data.chartConfig
...
/**
 * Author: John Handley
 * Purpose: Function to aquire Aggregate Data
 */
function aggregateDataFromTests(suiteId, testConfig, datapoints, chartConfig) {
  var chart = {
        suiteId: suiteId,
        testId: testConfig.testId,
        testDisplayName: testConfig.testDisplayName,
