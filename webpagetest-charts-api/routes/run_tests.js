...
function prepareTest(item, asyncCallback) {
  debug('preparing...');
  debug(item);
  var directory = item.testHost + item.path;
  //var script = jsonfile.readFileSync(directory);
  var fs = require('fs');
  var script = JSON.parse(fs.readFileSync(directory, 'utf8'));
  if(Object.keys(script).length === 0){
    console.log("BAD FILE");
  }
  debug(script);
  item.url = WebPageTest.scriptToString(script);
...
eventEmitter.on('runTests', function runTests(tests) {
  tests.forEach( function(test){
    setTimeout( function(){ runTest(test);}, getTestRunTimeout());
  });
});
...
function runTest(test) {
  debug('Starting test on \n' + test.url + '\n using ' + test.location);
  var testConfig = jf.readFileSync(process.env.SUITE_CONFIG)
    , wptPublic = new WebPageTest(testConfig.WebHost)
    , options = {
